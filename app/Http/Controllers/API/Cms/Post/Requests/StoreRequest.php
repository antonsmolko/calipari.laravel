...
class StoreRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'alias' => 'bail|required|unique:posts,alias|min:' . config('validation.alias.min') . '|max:' . config('validation.alias.max') . '|regex:' . config('validation.alias.pattern'),
            'date' => 'bail|required|integer',
            'intro' => 'bail|nullable|string',
            'image' => 'bail|required|file|image|mimes:jpeg,png|min:' . config('validation.upload.min_size.min') . '|max:' . config('validation.upload.max_size'),
